...
# -------------- Model SNGAN (Spectral Normalization GAN) --------------
...
class SNGANDiscriminator(nn.Module):
    def __init__(self, ndf=64):
        super().__init__()
        # apply spectral_norm to each conv
        self.net = nn.Sequential(
    ...
    def forward(self, img):
        # zwraca "score" bez aktywacji
        return self.net(img).view(-1)

# -------------- Hinge Loss Training for SNGAN --------------
...
def train_lsgan(dataloader, cfg, hyperparams={}, save_path="lsgan_history.pkl"):
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for real_imgs, _ in dataloader:
            ...
            real_labels = torch.full((bs,), 0.9, device=cfg.device)  # zamiast 1
            ...
            # Generator: chce, by d_fake było blisko 1
...
def interpolate_and_generate(gen, z1, z2, cfg, steps=10):
    ...
    # Poprawne przygotowanie tensorów
    zs = torch.stack([(1 - a) * torch.Tensor(z1) + a * torch.Tensor(z2) for a in alphas])
    zs = zs.view(steps, cfg.latent_dim, 1, 1).to(cfg.device)  # Kształt [10, 100, 1, 1]
